...
public class Pong_Server extends Server{
	...
	int pongLaenge = 100;
	int pongVerkleinerung = (int) (1 * pongLaenge) / (2 * scoreMax);
	int ballDicke = 15;
	int pongDicke = 15;
	...
	int lposAus = (int) (fensterY / 2) - (pongLaenge / 2);
	double rpos = (fensterY / 2) - (pongLaenge / 2);
	int rposAus = (int) (fensterY / 2) - (pongLaenge / 2);
...
//////WAND BERECHNUNG
	public void wandBerechnung() {
		if (lposAus <= -1) {
			lpos = 0;
			lposAus = 0;
		} else if (lposAus >= ((fensterY + 1) - pongLaenge + (scoreL * pongVerkleinerung))) {
			lpos = (fensterY - pongLaenge + (scoreL * pongVerkleinerung));
			lposAus = (int) lpos;
		}
		if (rposAus <= -1) {
			rpos = 0;
			rposAus = 0;
		} else if (rposAus >= ((fensterY + 1) - pongLaenge + (scoreR * pongVerkleinerung))) {
			rpos = (fensterY - pongLaenge + (scoreR * pongVerkleinerung));
			rposAus = (int) rpos;
		}
	}
	...
	public void richtungAendern() {
		if (posy >= (fensterY - ballDicke)) {
			...
			richtungv = 0;
				...
				&& ((posy - lposAus) < (pongLaenge - (scoreL * pongVerkleinerung)) && (posy - lposAus) > -ballDicke)) {
			richtungh = 0;
		} else if (posx <= (pongSeitenabstand + pongDicke)
				&& !((posy - lposAus) < (pongLaenge - (scoreL * pongVerkleinerung)) && (posy - lposAus) > -ballDicke)) {
			richtungh = 0;
			...
			if ((scoreL >= scoreMax) || (scoreR >= scoreMax)) {
				...
				&& ((posy - rposAus) < (pongLaenge - (scoreR * pongVerkleinerung)) && (posy - rposAus) > -ballDicke)) {
			richtungh = 1;
		} else if (posx >= (fensterX - (pongSeitenabstand + pongDicke + ballDicke))
				&& !((posy - rposAus) < (pongLaenge - (scoreR * pongVerkleinerung)) && (posy - rposAus) > -ballDicke)) {
			richtungh = 1;
